...
func TestQuotaBody(t *testing.T) {
	for _, tc := range []struct {
		th, window, want string
	}{
		{"75", "week", "You're at 75% of your weekly limit"},
		{"95", "day", "You're at 95% of your daily limit"},
		{"cap", "month", "You're over your monthly limit"},
		{"cap", "", "You're over your limit"}, // unknown window: no double space
	} {
		if got := quotaBody(tc.th, tc.window); got != tc.want {
			t.Errorf("quotaBody(%q,%q) = %q, want %q", tc.th, tc.window, got, tc.want)
		}
	}
}
...
func (f *fakeUsage) AccountWindows(context.Context) ([]AccountWindow, error) { return nil, nil }
